<template>
  <div>
    <a-collapse ghost>
      <a-collapse-panel key="1" header="圆形">
        <p>提供在地图图面绘制圆形覆盖物的能力。可以为圆形覆盖物设置填充颜色、描边颜色、轮廓线样式等属性。</p>
      </a-collapse-panel>
    </a-collapse>
    <a-card class="card" v-for="(item, index) in model.circles" :key="index">
      ...
      <a-row class="margin-top">
        ...
        <a-col :span="wrapperSpan">
          <a-input-number v-model:value="item.radius" :min="1" :max="99999" class="width-full" />
        </a-col>
      </a-row>
      <a-row class="margin-top">
        <a-col :span="labelSpan">
